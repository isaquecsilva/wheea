...
type WeatherQueryAdapter struct {
	meteo  openmeteo.OpenMeteo
	params openmeteo.Parameter
}

func NewWeatherQueryAdapter() *WeatherQueryAdapter {
	return &WeatherQueryAdapter{
		meteo: openmeteo.New(),
		params: openmeteo.Parameter{
...
func (wqa *WeatherQueryAdapter) Query(wq entities.WheatherQuery) entities.WheatherQueryApiResponse {
	...
	if rawresponse, err := wqa.meteo.Execute(wqa.params); err != nil {
		...
		case nil:
			...
			for index := 1; index <= len(response.Result.NextDays); index++ {

				response.Result.NextDays[index-1] = struct {
					WeatherType    uint
					MaxTemperature int
					MinTemperature int
					Precipitation  uint
